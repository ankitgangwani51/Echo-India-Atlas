({
    ...
    initializeVariables : function(component, event, helper) {
        ...
        var frameSrc = '/apex/' + this.setPrefixedString('BlAp_UploadPdfTemplatePage') + '?' + '&lcHost=' + component.get('v.lcHost');
        ...
        window.addEventListener("message", function(event) {
            if(event.data.eventType && atob(event.data.eventType) == 'custom'){
                ...
                // If state is loaded then setting value in vfhost.
                ...
                // If state is "invalidFileType"  then file type is not zip and setting  message in attribute.
                if(atob(event.data.state)== 'invalidFileType'){
                    component.set('v.fileTypeMessage', $A.get("$Label.c.BlAp_FileTypeZIPMessage"));
                }
                ...
                if(atob(event.data.state) == 'fileUploadprocessed'){
                   /*AT-4889 Code Check marx Changes Starts here*/
                    var returnMessage  = atob(event.data.message);
                   ...
                   /* $A.createComponents([
                    ...
                    if(returnMessage == $A.get("$Label.c.BlAp_FileUploadSuccesFullyMessage")){
    ...
    // Method is used to send values in vf page, 
    // Based on these parameter, need to upload the file and need to create xslt mapping
    doSave : function(component, event, helper) {
        if(this.validatePage(component, event, helper)){
            var xsltTemplateName  = component.get("v.xsltTemplateName");
    ...
    // Method is used to validate the page.
    validatePage: function(component, event, helper) {
        var errorMessages = [];
        var isValid= true;
        if(component.get("v.importFileName")){
            var isTemplateNotExist = this.checkValueExist(component.get("v.importFileName") , component.get("v.pdfTemplateList"))
            if(isTemplateNotExist){
                if(!component.get("v.xsltTemplateName")){
                    errorMessages.push($A.get("$Label.c.BlAp_XSLTMustBeProvided"));
                ...
                errorMessages.push($A.get("$Label.c.BlAp_PdfTemplateWithNameAlreadyExist"));
                this.showNotification(component, errorMessages,$A.get("$Label.c.GlAp_NotificationTypeError"));
                isValid = false;
            }
        }else{
    ...
    // checking in list if value exist or not .
    ...
    // Method is used to post message in vf page.
