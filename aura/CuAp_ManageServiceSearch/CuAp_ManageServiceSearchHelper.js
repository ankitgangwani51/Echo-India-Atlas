({
    ...
    setServiceTableBlank: function(component, event, helper) {
        ...
        component.set('v.selectedSupplyPointId',null)
    ...
    getAllServices: function(component, event, helper) {
        ...
        var spLookup 			= component.get('v.selectedSupplyPointId');
        ...
        if(recordBy == $A.get('$Label.c.GlAp_SupplyPointLabel') && !spLookup){
            this.showNotification(component, [$A.get("$Label.c.CuAp_SelectSupplyPoint")],'error');   
            return false;
        } 
        
        if(recordBy != $A.get('$Label.c.GlAp_ContractLabel')){
            callingServiceCmp.getServiceRelatedInfo(null,component.get('v.selectedSupplyPointId').Id);
        }else
    ...
    doInit: function(component, event, helper){          
        ...
        const operationVal = [
            ...
            $A.get('$Label.c.GlAp_SupplyPointLabel')            
        ];
    ...
    validateServiceSearch: function(component, event, helper){
        ...
        for(var i=0;i<recordList.length;i++){
            if(recordList[i].isSelected){
                ...
                if(recordBy == $A.get('$Label.c.GlAp_ContractLabel')){
                    ...
                    rec = {sobjectType: this.setPrefixedString('AvailableService__c'), 
                           ['Id']										: recordList[i].uniqueId,
                           [this.setPrefixedString('ServiceType__c')]	: recordList[i].objectMap[this.setPrefixedString('AvailableService__c')][this.setPrefixedString('ServiceType__c')],
                           [this.setPrefixedString('SupplyPoint__c')]	: recordList[i].objectMap[this.setPrefixedString('AvailableService__c')][this.setPrefixedString('SupplyPoint__c')]}
                    newRecordList.push(rec);                    
                }
            }
        }
        ...
        if(!recSelected){
            if(recordBy == $A.get('$Label.c.GlAp_ContractLabel'))
                ...
                this.showNotification(component, [$A.get("$Label.c.CuAp_SelectOneAvailableService")],'error');   
            return false;
        }        
        ...
        if(recordBy == $A.get('$Label.c.GlAp_ContractLabel')){
            ...
            wizObj.availableServiceTransfers = null;
        }
        else{
            wizObj.serviceTransfers = null;
            wizObj.availableServiceTransfers = newRecordList;
        }
