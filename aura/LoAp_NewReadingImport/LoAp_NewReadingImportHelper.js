({ 
    ...
    handleFilesChange: function(component, event, helper) {
        ...
        if(file != undefined){  //AT-3039
        	component.set("v.fileName", file.name);
            component.set("v.fileDetails",file);      // AT-3039
        }
    },
	...
	doSave : function(component, event, helper) {       
        ...
        if(file == undefined){							// AT-3039
            file = component.get("v.fileDetails");      // AT-3039
            if(!file){	
        ...
        if( filename.lastIndexOf('.csv') < 0 || (filename.lastIndexOf('.csv') > 0 && filename.substring(filename.lastIndexOf('.csv'), filename.length) != '.csv')){
            this.showNotification(component, [$A.get("$Label.c.LoAp_CSVFileType")],$A.get('$Label.c.GlAp_NotificationTypeError'));
            return false; 
        }
