({
	getServiceRelatedInfo : function(component, event, helper) {
		return helper.getServiceRelatedInfo(component, event, helper);
	},
    // Row select Event method
    handleRowSelectEvent: function(component, event, helper) {
        return helper.handleRowSelectEvent(component, event);
    },
    getAUServiceRelatedInfo : function(component, event, helper) {
		return helper.getAUServiceRelatedInfo(component, event, helper);
	},
})